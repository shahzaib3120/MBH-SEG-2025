...
def plot_overlay_preprocessed(
	...
	assert channel_idx < (data.shape[0]), 'This dataset only supports channel index up to %d' % (
		data.shape[0] - 1
	)
...
def multiprocessing_plot_overlay_preprocessed(
	list_of_case_files, list_of_output_files, overlay_intensity, num_processes=8, channel_idx=0
):
	with multiprocessing.get_context('spawn').Pool(num_processes) as p:
		r = p.starmap_async(
...
def generate_overlays_from_raw(
	...
	multiprocessing_plot_overlay(
		image_files, seg_files, image_reader_writer, output_files, overlay_intensity, num_processes
	)
...
def entry_point_generate_overlay():
	...
	if args.use_raw:
		generate_overlays_from_raw(
			args.d, args.o, args.np, args.channel_idx, overlay_intensity=args.overlay_intensity
		)
	else:
