...
class nnUNetTrainer(object):
	def __init__(
		...
		self.preprocessed_dataset_folder = join(
			self.preprocessed_dataset_folder_base, self.configuration_manager.data_identifier
		)
	...
	def _do_i_compile(self):
		...
		if self.device == torch.device('mps'):
			if 'nnUNet_compile' in os.environ.keys() and os.environ['nnUNet_compile'].lower() in (
				'true',
				'1',
				't',
			):
	...
	def _get_deep_supervision_scales(self):
		if self.enable_deep_supervision:
			deep_supervision_scales = list(
				list(i)
				for i in 1
				/ np.cumprod(np.vstack(self.configuration_manager.pool_op_kernel_sizes), axis=0)
			)[:-1]
		else:
	...
	def _set_batch_size_and_oversample(self):
		if not self.is_ddp:
			...
			if sample_id_high / global_batch_size < (1 - self.oversample_foreground_percent):
				...
				oversample_percent = (
					sum(oversample[sample_id_low:sample_id_high]) / batch_size_per_GPU[my_rank]
				)
	...
	def on_train_start(self):
		...
		save_json(
			self.plans_manager.plans, join(self.output_folder_base, 'plans.json'), sort_keys=False
		)
