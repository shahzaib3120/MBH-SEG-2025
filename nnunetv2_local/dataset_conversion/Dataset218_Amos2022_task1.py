...
def convert_amos_task1(amos_base_dir: str, nnunet_dataset_id: int = 218):
	...
	for tr in training_identifiers:
		if int(tr.split('_')[-1]) <= 410:  # these are the CT images
			tr_ctr += 1
			shutil.copy(
				join(amos_base_dir, 'imagesTr', tr + '.nii.gz'), join(imagestr, f'{tr}_0000.nii.gz')
			)
			shutil.copy(
				join(amos_base_dir, 'labelsTr', tr + '.nii.gz'), join(labelstr, f'{tr}.nii.gz')
			)
	...
	for ts in test_identifiers:
		if int(ts.split('_')[-1]) <= 500:  # these are the CT images
			shutil.copy(
				join(amos_base_dir, 'imagesTs', ts + '.nii.gz'), join(imagests, f'{ts}_0000.nii.gz')
			)
	...
	for vl in val_identifiers:
		if int(vl.split('_')[-1]) <= 409:  # these are the CT images
			tr_ctr += 1
			shutil.copy(
				join(amos_base_dir, 'imagesVa', vl + '.nii.gz'), join(imagestr, f'{vl}_0000.nii.gz')
			)
			shutil.copy(
				join(amos_base_dir, 'labelsVa', vl + '.nii.gz'), join(labelstr, f'{vl}.nii.gz')
			)
...
if __name__ == '__main__':
	...
	parser.add_argument(
		'-d', required=False, type=int, default=218, help='nnU-Net Dataset ID, default: 218'
	)
