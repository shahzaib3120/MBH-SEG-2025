...
def save_cardiac_phases(
	...
	for patient in patients:
		...
		save_extracted_nifti_slice(
			image, ed_frame=ed_frame, es_frame=es_frame, out_dir=out_dir, patient=patient
		)

		if labels_dir:
			label = nib.load(patient / f'{patient.name}_sa_gt.nii.gz')
			save_extracted_nifti_slice(
				label, ed_frame=ed_frame, es_frame=es_frame, out_dir=labels_dir, patient=patient
			)
...
def create_custom_splits(
	...
	patients_train = [
		f.name for f in (src_data_folder / 'Training' / 'Labeled').iterdir() if f.is_dir()
	]
	...
	patients_a = [
		patient for patient, patient_data in patient_info.items() if patient_data['vendor'] == 'A'
	]
	patients_b = [
		patient for patient, patient_data in patient_info.items() if patient_data['vendor'] == 'B'
	]
