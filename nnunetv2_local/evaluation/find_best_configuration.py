...
def generate_inference_command(
	...
	trained_model_folder = get_output_folder(
		dataset_name_or_id, trainer_name, plans_identifier, configuration_name, fold=None
	)
		...
		for i in range(len(allowed_trained_models)):
			for j in range(i + 1, len(allowed_trained_models)):
				...
				output_folder_1 = get_output_folder(
					dataset_name_or_id, m1['trainer'], m1['plans'], m1['configuration'], fold=None
				)
				output_folder_2 = get_output_folder(
					dataset_name_or_id, m2['trainer'], m2['plans'], m2['configuration'], fold=None
				)
				...
				all_results[identifier] = {
					...
					'result': load_summary_json(join(output_folder_ensemble, 'summary.json'))[
						'foreground_mean'
					]['Dice'],
				}
...
def find_best_configuration_entry_point():
	...
	parser.add_argument(
		'-f', nargs='+', type=int, default=(0, 1, 2, 3, 4), help='Folds to use. Default: 0 1 2 3 4'
	)
	parser.add_argument(
		'--disable_ensembling',
		action='store_true',
...
def accumulate_crossval_results_entry_point():
	...
	parser.add_argument('dataset_name_or_id', type=str, help='Dataset Name or id')
	parser.add_argument('-c', type=str, required=True, default='3d_fullres', help='Configuration')
	parser.add_argument(
		'-o',
		type=str,
		required=False,
	...
	if args.o is None:
		merged_output_folder = join(
			trained_model_folder, f'crossval_results_folds_{folds_tuple_to_string(args.f)}'
		)
	else:
