...
def determine_postprocessing(
	...
	with multiprocessing.get_context('spawn').Pool(num_processes) as pool:
		...
		do_this = (
			pp_results['foreground_mean']['Dice'] > baseline_results['foreground_mean']['Dice']
		)
		if do_this:
		...
		if len(labels_or_regions) > 1:
			for label_or_region in labels_or_regions:
				...
				if do_this:
					...
					if isdir(
						join(output_folder, 'temp', 'keep_largest_perClassOrRegion_currentBest')
					):
						shutil.rmtree(
							join(output_folder, 'temp', 'keep_largest_perClassOrRegion_currentBest')
						)
					...
					source = join(
						output_folder, 'temp', 'keep_largest_perClassOrRegion_currentBest'
					)
					pp_fns.append(pp_fn)
					pp_fn_kwargs.append(kwargs)
				else:
...
def entry_point_determine_postprocessing_folder():
	...
	determine_postprocessing(
		args.i, args.ref, args.plans_json, args.dataset_json, args.np, not args.remove_postprocessed
	)
...
def entry_point_apply_postprocessing():
	...
	apply_postprocessing_to_folder(
		args.i, args.o, pp_fns, pp_fn_kwargs, args.plans_json, args.dataset_json, args.np
	)
