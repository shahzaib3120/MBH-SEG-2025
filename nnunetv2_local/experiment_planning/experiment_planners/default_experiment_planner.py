...
class ExperimentPlanner(object):
	def __init__(
		...
		self.max_dataset_covered = (
			0.05  # we limit the batch size so that no more than 5% of the dataset can be seen
		)
	...
	def get_plans_for_configuration(
		...
		max_num_features = (
			self.UNet_max_features_2d if len(spacing) == 2 else self.UNet_max_features_3d
		)
	...
	def plan_experiment(self):
		...
		new_shapes = [
			...
			for i, j in zip(
				self.dataset_fingerprint['spacings'], self.dataset_fingerprint['shapes_after_crop']
			)
		]
		...
		if new_median_shape_transposed[0] != 1:
			...
			while num_voxels_in_patch / median_num_voxels < self.lowres_creation_threshold:
				...
				plan_3d_lowres = self.get_plans_for_configuration(
					...
					float(np.prod(median_num_voxels) * self.dataset_json['numTraining']),
					_tmp,
				)
