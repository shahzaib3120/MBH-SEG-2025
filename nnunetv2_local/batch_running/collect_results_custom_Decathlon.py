							...
							if isdir(expected_output_folder):
								...
								for fl in folds:
									...
									expected_summary_file = join(
										expected_output_folder_fold, 'validation', 'summary.json'
									)
									if not isfile(expected_summary_file):
										print(
											'expected output file not found:', expected_summary_file
										)
										f.write(',')
										results_folds.append(np.nan)
									else:
										foreground_mean = load_summary_json(expected_summary_file)[
											'foreground_mean'
										]['Dice']
